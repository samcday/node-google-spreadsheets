...
var ATTR = {
    ...
    'xmlns': "http://schemas.google.com/spreadsheets/2006/extended"
};
...
var addToFeed = function (params, auth, body, cb) {
    var headers = {};
    ...
    var url = FEED_URL + (params.concat([ visibility, projection]).join("/"))+"?alt=json";
    body._attr = ATTR;

    request.post(url, {
        headers: headers,
        body: data2xml('atom:entry', body)
        ...
        if (response.statusCode >= 400) {
            return cb(new Error("HTTP error " + response.statusCode + ": " + http.STATUS_CODES[response.statusCode] + " " + body));
        }
...
Worksheet.prototype.addRow = function (row, cb) {

    addToFeed(['list', this.spreadsheet.key, this.id], this.spreadsheet.auth, row, cb);
}
